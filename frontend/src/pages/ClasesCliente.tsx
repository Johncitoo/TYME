...
import { getClasesDisponibles, inscribirAsistencia, cancelarAsistencia, getAsistenciasCliente } from "@/services/class.service";
...
export default function ClasesCliente() {
  ...
  const estaInscrito = (id_clase: number) => asistencias.find((a) => a.clase.id_clase === id_clase);
  const claseLlena = (clase: Clase) => clase.cupos_ocupados >= clase.cupo_maximo;
  ...
  const handleDesinscribir = async (id_asistencia: number) => {
    try {
      await cancelarAsistencia(id_asistencia);
        ...
        {loading ? (
          ...
          <ul className="flex flex-col gap-3">
            {clasesFuturas.map((clase) => {
              const inscrito = estaInscrito(clase.id_clase);
              const llena = claseLlena(clase);
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-1 text-zinc-400 text-xs mb-1">
                    ...
                    {clase.entrenador?.usuario?.primer_nombre ?? "Entrenador"} • Cupos: {clase.cupos_ocupados} / {clase.cupo_maximo}
                  </div>
                  <div className="flex justify-end">
                    {inscrito ? (
                      <Button
                        ...
                        onClick={() => handleDesinscribir(inscrito.id_asistencia)}
                      >
                      ...
                      <Button
                        size="sm"
                        className="bg-red-800 text-red-100 font-bold rounded-lg px-4 py-2 cursor-not-allowed"
                        disabled
                      >
                        Clase llena
                      </Button>
