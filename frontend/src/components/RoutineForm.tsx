// frontend/src/components/RoutineForm.tsx
...
type Props = {
  idEntrenador?: number; // Si se pasa, el select entrenador se oculta y se usa ese valor
  idCliente?: number;    // Si se pasa, el select cliente se oculta y se usa ese valor
  onSuccess?: () => void;
};

const RoutineForm: React.FC<Props> = ({ idEntrenador, idCliente, onSuccess }) => {
  // Datos básicos
  const [nombre, setNombre] = useState('');
  const [descripcion, setDescripcion] = useState('');
  const [selEntrenador, setSelEntrenador] = useState<number | undefined>(idEntrenador);
  const [selCliente, setSelCliente] = useState<number | undefined>(idCliente);
  ...
  useEffect(() => {
    ...
    if (!idCliente) {
      getAllClientes().then(setClientes).catch(() => setError('No se pudieron cargar los clientes'));
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setError(null);
    const entrenadorId = idEntrenador ?? selEntrenador;
    const clienteId = idCliente ?? selCliente;
    if (!nombre || !entrenadorId || !clienteId || !year || !month || !day) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded shadow space-y-6 max-w-4xl mx-auto">
